            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Badge variant={conditionColor(exp.condition)} className="text-xs">
                    {exp.condition}
                  </Badge>
                  {exp.question_type && (
                    <span className="text-xs text-gray-400">{exp.question_type}</span>
                  )}
                  <span className="text-xs text-gray-400">
                    {new Date(exp.timestamp).toLocaleString()}
                  </span>
                </div>
                <p className="text-sm text-gray-700 truncate">{exp.question_body}</p>
                {exp.answer && (
                  <p className="text-xs text-gray-500 mt-1 line-clamp-2">{exp.answer}</p>
                )}
              </div>
              <div className="text-right text-xs text-gray-400 shrink-0">
                ...
                {exp.metrics && (
                  <div className="mt-1 space-y-0.5">
                    <p>MAP: {(exp.metrics.retrieval.map_at_k * 100).toFixed(0)}%</p>
                    <p>Task: {(exp.metrics.task.score * 100).toFixed(0)}%</p>
                  </div>
                )}
              </div>
            </div>
